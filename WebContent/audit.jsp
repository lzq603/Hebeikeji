...
<%
	...
	List<Platform> platforms = platformDao.loadWhere("Pstatus = 2");
...
$.each(json_platform,function(key,obj){
	console.log(obj);
	obj.check = "<a href='check.jsp'>查看</a>";
	obj.audit = "<a href='javascript:;' onclick=audit(" + obj.id + ",3)>审核通过</a>|<a href='javascript:;' onclick=audit(" + obj.id + ",4) >不通过</a>";
	...
	layui.use('table', function(){
		...
		table.render({
		    ...
		    ,cols: [[ //表头
		      ...
		      ,{field: 'check', title: '查看', width:100}
		      ,{field: 'audit', title: '审核'}
		    ]]
		});
	});
	function audit(Pid,status){
		$.ajax({
			url:'AutidServlet?check=' + status + '&id=' + Pid,
			type:'GET',
			dataType:'json',
